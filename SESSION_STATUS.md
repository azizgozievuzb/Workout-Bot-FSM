...
## ⏳ НЕ сделано (ждёт)
**Живой смоук 8d.1** — чек-лист в `PROMPT_8D1.md` (14 пунктов). Обязательно проверить плеер и «Скачать» НА iPhone: видео на iOS-webview уже ловили дважды (S57). Пункт «AI-стилизация» смоука теперь проверяет уже оттюненные промпты.

⚠️ **`backend/.env` (создан юзером для прогона) удалить после сессии** — правило S57, в git не попадал (`.gitignore:20`).
