...
## ▶️ Следующая точка
**Патч 8c.1 (сразу, решения юзера со смоука):** №12 прогрессирующие цены AI-смен (raw — фикс; AI №k = photo_card×2^(k−1) с капом ~3200, счётчик на юзере; реролл №m = photo_reroll×2^(m−1), счётчик сбрасывается сменой; множитель/кап админ-редактируемые в meta; на витрине — актуальная для юзера цена), №13 селфи-капчер getUserMedia (живое превью + «Снять», переиспользовать камерную инфру тренировок; input capture — мобильный шорткат; файловый пикер — фолбэк), №14 double-submit guard на ВСЕХ кнопках покупок + идемпотентность бэка к двойному рероллу. После 8c.1 — Cowork готовит PROMPT_8D. Также на контроле: первая живая ночь closure (см. хотфикс 7b31630) и фикс чат-счёта продления (предзапуск).
