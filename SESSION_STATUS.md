...
**Для новой Cowork-сессии (заметка Cowork, 2026-07-25):** дизайн-ревизия §8 завершена (решения — S50 + BACKLOG §8 v2), 8a/8b реализованы и просмоучены, 8c реализована. Порядок: живой смоук 8c (ведёт CC; остановлен 2026-07-26 на пункте 4 шаг 4.1 — ранний выход 2/4; CC подготовил light-день, откат-список расширен) → патч 8c.1 силами CC (прогрессирующие цены AI ×2^(k−1), селфи-капчер getUserMedia, double-submit guard) → затем Cowork готовит PROMPT_8D (полка наставника: видео-обещания полным циклом, Stars-предметы, слоты 2/4/6, gift_balance/drop packs/дарение — всё спроектировано в S50 п.5/§8.8a). После 8d: 8e (полировка, UX-долги №1–№7 + гендер-словари) → экономическая сверка ECONOMICS.html → контент-треки юзера (демо light/main, мультяшки) и предзапускной чек-лист — отдельно.
