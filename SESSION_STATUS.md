...
- **Хвосты:** 4 видеофайла в приватном бакете `promises` (3.25/7.47/4.55/0.65 МБ, префикс `ebd7b00a-…`) — Supabase запрещает `DELETE` из `storage.objects` (защита от орфанов), чистятся только через Storage API/дашборд. Плюс `card_v_*` и `card_selfie.jpg` в `avatars`, как и после смоука 8c.
