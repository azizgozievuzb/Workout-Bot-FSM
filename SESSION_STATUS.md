# SESSION STATUS — Session 59 (2026-08-03…05, CC+юзер) — СМОУК 8d.1: 14/14 ✅, 15 хотфиксов, откат сверен
...
## 🧹 ХВОСТ: орфаны в Storage (не вычищены, нужен service-ключ)
Прямой `DELETE` из `storage.objects` Supabase запрещает — чистить HTTP-эндпоинтом с service-ключом (как в S57, `backend/.env` на время работы). **13 файлов, ≈8.2 МБ:**
- бакет `promises` (3 файла, 3.3 МБ): `promise_118bfb7f…` (от снесённого «куни»), `promise_dc25e3f4…` (обещание смоука), `report_84be043e…` (отчёт смоука);
...
## 📦 Готовится Cowork-ом: пакет 8d.1a
Отличия строк игрока по кубам (Bond компактнее и т.п.) и правки состава досье — сознательно НЕ вносились в смоук, идут отдельным мини-патчем.
