...
## 🧹 ОТКРЫТЫЙ ХВОСТ: орфаны в Storage — НЕ вычищены, нужен service-ключ
Прямой `DELETE` из `storage.objects` Supabase запрещает (защита от орфанов) — чистить HTTP-эндпоинтом с service-ключом, как в S57: юзер кладёт `SUPABASE_SERVICE_KEY` в `backend/.env`, CC чистит, файл удаляется. **10 файлов, ≈7.5 МБ:**
- бакет `promises` (3 файла, 3.3 МБ): `promise_118bfb7f…` (от снесённого лота «куни»), `promise_dc25e3f4…` (обещание смоука), `report_84be043e…` (отчёт смоука);
- `avatars/8777447186/` (7 файлов фото-карточки, 4.2 МБ): `card_selfie.jpg` + `card_v_bcaa0671` / `61f1bc0c` / `c221afa1` / `c82fbf76` / `1bd0d60a` / `1a51cd59`.

**Решение юзера 2026-08-05 по `card_v_1a51cd59.jpg`:** фото-карточку Cell, поставленную в ходе смоука, НЕ возвращать (не боевое фото) — файл удаляется вместе с остальными орфанами. В БД она уже обнулена откатом.

**НЕ трогать** в той же папке: `bbb9c474….jpg`, `thumb.jpg`, `dark.jpg`, `light.jpg` — рабочие аватары.
