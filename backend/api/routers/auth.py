...
USER_SELECT_COLS = (
    ...
    "ban_until, ban_reason, ban_missed_workouts, access_tier, "
...
async def _build_full_token_response(db, telegram_id: int, user_data: dict) -> TokenResponse:
    ...
    if is_admin or has_responsible_access:
        own_access_tier = user_data.get("access_tier")

    # player_view_tier + days_left — from own partnership
    player_view_tier: str | None = None
    days_left: int | None = None
    if has_player_access:
        part_res = (
            await db.table("partnerships")
            .select("expires_at, responsible_id")
        ...
        if part_row:
            resp_id = part_row.get("responsible_id")
            if resp_id:
                resp_res = (
                ...
                if resp_res is not None and resp_res.data:
                    player_view_tier = resp_res.data.get("access_tier")
