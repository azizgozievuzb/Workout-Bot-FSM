-- ============================================================
-- Migration 036b: Phase 8c — атомарные RPC покупок
--   * buy_paid_freeze  — списание капель + paid_freezes+1 (кап 3) одним UPDATE.
--   * restore_streak   — восстановление стрика после слома (окно проверяет
--     вызывающий код; RPC гарантирует атомарность баланс+стрик).
...
-- p_expected_len защищает от гонки read→RPC (цена считалась по этой длине).
