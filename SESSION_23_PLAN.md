...
### 🆕 Tier Downgrade — авто-очистка лишних партнёрств (TODO, Этап 5)
14. При активации R-кода с пониженным тиром (Elite→Premium, Elite→Standard, Premium→Standard): если число истёкших партнёрств > новый slot_limit → система обязана удалить лишние. Логика:
    - Если `resurrect_partnership_id` передан → воскресить одного, удалить остальных истёкших.
    - Если НЕ передан, а истёкших > slot_limit → вернуть 422 `CHOOSE_RESURRECT_TARGET` с списком игроков для выбора. UI должен показать модалку выбора.
    - Если истёкших ≤ slot_limit → все остаются «спящими» (штатная логика).

### 🆕 Смена Ответственного (TODO, Этап 5)
15. Если Игрок с истёкшим партнёрством активирует P-код другого Ответственного:
    - Автоматически hard-delete всех его истёкших партнёрств (cleanup старых связей).
    - Создать новое активное партнёрство с новым Ответственным.
    - Если старое партнёрство было единственным и не-dual-role → удалить player row и пересоздать чисто (как новый игрок).
    - Реализовать в `_activate_player_code` в `promo.py` (5-10 строк).
