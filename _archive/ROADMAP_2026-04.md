# ☠️ АРХИВ (S61, 2026-08-07): апрельский учебный план. НЕ ИСТОЧНИК ПРАВДЫ

> Файл заморожен на апреле-2026 и описывает несуществующее: бусты X2 (выпилены в 8d), оплату промокодами (промо = купон скидки с 7.5), `shop_items` с лутбоксами (сейчас `app_shop_items`), `star_balance` (переименован в XP), NOTIFICATION_SETTINGS и «пинг раз в 6 ч» (не существуют), aiohttp (бэк на FastAPI), структуру `backend/bot/` (реально `backend/api/…`).
> Схему БД смотреть в живой БД (`my-supabase` → `list_tables`) или в коде. Процесс — `PLAYBOOK.md`. Оставлен как исторический документ эпохи проектирования.
...
| Капли 💧 (внутренняя валюта, ex-«Звёзды») | ✅ | ✅ | ✅ |
| Стрики (серии дней) | ✅ | ✅ | ✅ |
...
- drops_earned (INTEGER) — Капли 💧 за тренировку (миграция 026, Session 37). Внутриигровая валюта, НЕ Telegram Stars.
...
| Responsible | Player закончил тренировку | "✅ Партнёр закончил! XP: 87/100. Капли 💧: +42" |
...
- Суммарный score конвертируется в **Капли 💧** (внутриигровая валюта, ex-«Звёзды»; не путать с Telegram Stars)
- Формула: `drops = round(min(50 × quality × completion × streak_mult, 50))` (см. `backend/core/workout_config.py`)
- Если активен буст X2 → drops × 2
- Капли = валюта магазина
...
- [ ] Капли 💧 начисляются правильно (формула: 50 × quality × completion × streak_mult)
- [ ] Буст X2 удваивает капли
